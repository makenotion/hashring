describe("HashRing", function () {
	...
	it("exposes the library version", function () {
		assume(Hashring.version).matches(/^\d+\.\d+\.\d+-notion$/)
	})
	...
	describe("API", function () {
		...
		it("constructs with a default vnode value", function () {
			var ring = new Hashring(
				["192.168.0.102:11212", "192.168.0.103:11212"],
				"md5",
				{ "vnode count": 60 }
			)
		...
		describe("#get", function () {
			it("looks up keys", function () {
				...
				assume(
					ring.find(ring.hashValue("привет мир, Memcached и nodejs для победы"))
				).is.above(-1)
				...
				assume(ring.get({ wtf: "lol" })).equals(
					ring.get({ wtf: "amazing .toStringing" })
				)
			})
		})
		...
		describe("#find", function () {
			it("should find the correct long value for a given key", function () {
				...
				assume(ring.ring[ring.find(ring.hashValue("test"))].value).equals(
					3454255383
				)
			})
		})
		...
		describe("#remove", function () {
			...
			it("removes two servers", function () {
				var ring = new Hashring([
					"192.168.0.102:11212",
					"192.168.0.103:11212",
					"192.168.0.104:11212",
				...
				assume(ring.servers).has.length(1)
				assume(ring.ring.length).is.above(1)
				// show still have one, 105 is not a vaild one
				ring.ring.forEach(function (node) {
					assume(node.server).does.not.equal("192.168.0.102:11212")
					assume(node.server).does.not.equal("192.168.0.103:11212")
				})
			})
